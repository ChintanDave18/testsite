...
4. Wait a minute or two and reload that screen — the live URL appears at the top.
...
- **Colors**: the six values under `:root` in `style.css` drive every page. `--rani` is the hero, `--marigold` the buttons, `--indigo` and `--emerald` the alternate bands.
